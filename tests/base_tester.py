...
def expect(expected_code: int, expected_reason: str = ""):
    def decorator(func):
        def wrapper(self, *args, **kwargs) -> tuple[bool, str]:
            try:
...
def get_available_contact_name(client: EPPClient):
    return try_until_success(
        generate_random_name,
        lambda name: f'<contact:reason>Object exists</contact:reason>' not in client.contact_check(name),
        length=10)

def get_available_host_name(client: EPPClient, suffix=".epp.ua"):
    return try_until_success(
        generate_random_name,
        lambda name: f'<host:reason>Object exists</host:reason>' not in client.host_check(name),
        length=10, suffix=suffix)

def get_available_domain_name(client: EPPClient, suffix=".epp.ua", length=10):
    return try_until_success(
        generate_random_name,
        lambda name: f'<domain:reason>Object exists</domain:reason>' not in client.domain_check(name),
...
class TestContext:
    def __init__(self, login: str, password: str, certfile: str, keyfile: str):
        ...
        test_contacts = [get_available_contact_name(self.client) for _ in range(9)]
        self._check_or_create_contacts(test_contacts)
        self.test_contacts: list = test_contacts[:]

        test_domains = [get_available_domain_name(self.client) for _ in range(1)]
        self._check_or_create_domains(test_domains)
        self.test_domains: list = test_domains[:]

        test_hosts = [get_available_host_name(self.client,suffix="."+domain) for domain in test_domains]
        self._check_or_create_hosts(test_hosts)
        self.test_hosts: list = test_hosts[:]

        self.contacts_to_delete = test_contacts[:]
        self.domains_to_delete = test_domains[:]
        self.hosts_to_delete = test_hosts[:]
    ...
    def _check_or_create_domains(self, domains: list):
        for domain in domains:
            ...
            if f'<domain:name avail="0">{domain}</domain:name>' not in response:
                response = self.client.domain_create(domain, 1,[],self.test_contacts[0],[])
...
class Tester:
    def __init__(self, context: TestContext):
        ...
        self.test_contacts: list = context.test_contacts
        self.test_domains: list = context.test_domains
        self.test_hosts: list = context.test_hosts
    ...
    def safe_create_contact(self):
        name = get_available_domain_name(self.client)
    ...
    def safe_create_host(self, suffix=".epp.ua") -> str:
        name = get_available_domain_name(self.client,suffix)
